...
vector<BN> trial_divisor_method(const BN& n){
    ...
    while (check_num != one){ // 2
        if (check_num % 2 != zero){
            BN r = check_num % d[k]; // 3
            BN q = check_num / d[k];
            if (r == zero){ // 4
                t++;
                p.push_back(d[k]);
...
int main() {
    ...
    if (test_miller_rabin(num, test)) {
        cout << "Число простое. Проверка была совершена с помощью алгоритма Миллера-Рабина\n";
